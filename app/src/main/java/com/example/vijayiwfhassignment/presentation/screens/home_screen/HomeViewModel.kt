...
class HomeViewModel(private val repository: TitlesRepository) : ViewModel() {
    ...
    init {
        fetchTitlesWithDetails()
    }

    private fun fetchTitlesWithDetails() {
        ...
        val disposable = repository.getMoviesAndTvShows()
            ...
            .subscribe(
                { (movies, tvShows) ->
                    _state.value = HomeUiState.Success(movies, tvShows)
                },
                { throwable ->
